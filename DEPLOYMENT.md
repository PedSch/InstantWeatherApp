...
npm run dev
```
Runs local dev server at http://localhost:5173

### Staging
```bash
npm run build:staging
```
Builds with `.env.staging` configuration

### Production
```bash
npm run build:production
```
Builds with `.env.production` configuration
...
| `VITE_MAP_PROVIDER_BASE` | No | Custom map provider URL |
