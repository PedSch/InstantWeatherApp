...
export default function DailyForecast({ data = [], unit = 'C' }) {
  ...
  return (
    <div>
      <h2 className="text-sm font-medium text-slate-600 mb-3">{t('daily_forecast')}</h2>
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 sm:gap-4">
        {data.map((d, i) => (
          <div key={i} className="bg-white/80 rounded-lg p-3 sm:p-4 shadow-sm flex flex-col items-center card-transition hover:scale-[1.02]">
