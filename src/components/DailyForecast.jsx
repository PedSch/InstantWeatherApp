  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {data.map((d, i) => (
          <div key={i} className="bg-white/80 rounded-lg p-3 shadow-sm flex flex-col items-center card-transition hover:scale-[1.02]">
            <div className="text-sm text-slate-500">{formatDay(d.date)}</div>
            <div className="text-2xl mt-1">{getIcon(d.code, 1, 'w-8 h-8')}</div>
